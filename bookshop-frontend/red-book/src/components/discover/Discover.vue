...
const displayedCount = ref(20); // 当前显示的文章数量
...
const displayArticles = computed(() => {
  ...
  // 否则返回articleLists的前displayedCount条
  return articleLists.value.slice(0, displayedCount.value);
});

// 是否还有更多数据可以加载
const hasMoreData = computed(() => {
  if (isSearch.value) {
    return displayedCount.value < filteredArticles.value.length;
  } else {
    return displayedCount.value < articleLists.value.length;
  }
});
...
const handleScroll = () => {
  ...
  const { scrollTop, scrollHeight, clientHeight } = mainBody;
    ...
    <transition name="fade">
      <div class="main-body">
        ...
        <template v-if="(isSearch && searchArticle) || !isSearch">
          ...
          <div v-else-if="hasMoreData" class="load-more-tip">滚动加载更多</div>
          <div v-else-if="displayArticles.length > 0" class="no-more-data">没有更多数据了</div>
        </template>
